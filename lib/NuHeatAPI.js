...
module.exports = class NuHeatAPI {
    ...
    async setHeatSetpoint(serialNumber, setPointTemp, holdLength) {
        ...
        if (holdLength < 1440) {
            scheduleMode = 2;
            if (holdLength > 0) {
                let holdSetPointDateTime = new Date(Date.now() + holdLength*60*1000).toISOString().split('.')[0] + 'Z';
            }
        }
    ...
    async makeAPICall(callURL, callOptions = {}) {
        ...
        // Parse data into JSON safely
        let data = await response.text();
        try {
          let returnedData = JSON.parse(data);
          return(returnedData);
        } catch (e) {
          this.log.warn("NuHeatAPI: Unable to parse JSON from response.");
          return(null);
        }
    };
